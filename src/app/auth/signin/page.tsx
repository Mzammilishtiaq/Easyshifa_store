...
export default function Page() {
  ...
  const handleSubmited = async (event: React.FormEvent) => {
    ...
    try {
      const response = await axios.post<{ success: boolean, token: string, message: string }>('http://localhost:8000/api/user/login', { email, password });
console.log(response)
  ...
  return (
    <div className='flex flex-col items-center justify-center my-10'>
      <form onSubmit={handleSubmited} className='shadow-2xl rounded-2xl p-3 w-[30%] space-y-2 flex flex-col'>
